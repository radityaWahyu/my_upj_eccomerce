    ...
    <div class="lg:w-3/4 mx-auto space-y-4">
      ...
      <div>
        <blockquote class="mt-6 border-l-2 pl-6 italic text-sm">
          <strong>Keterangan :</strong> Silahkan cek kembali produk / jasa yang ingin
          dipesan melalui sistem ini, sebelum melakukan proses selesaikan pemesanan.
        </blockquote>
      </div>
      <div class="space-y-2" v-if="carts !== null && carts.data.length > 0">
        <ProductCart
        ...
        <div class="w-full space-x-2 text-center">
          <button
            ...
            @click="emptyCarts"
          >
            <span v-if="isEmptyCartLoading">Mengosongkan Keranjang...</span>
          ...
          <button
            ...
            :disabled="isCheckOutLoading || isEmptyCartLoading"
          >
            <span v-if="isCheckOutLoading">Mengirim Pesanan...</span>
            <span v-else>Kirim Pesanan</span>
          </button>
        </div>
      </div>
      <div v-if="carts !== null && carts.data.length === 0" class="w-full">
        <img :src="EmptyCarts" alt="empty_cart" class="w-80 h-80 mx-auto" />
        <h2
      ...
      <div v-if="carts === null" class="w-full">
        ...
        <h2
          class="scroll-m-20 border-b pb-2 text-xl font-semibold tracking-tight transition-colors text-center"
        >
          Anda harus LogIn terlebih dahulu agar dapat melihat keranjang belanja anda.
        </h2>
