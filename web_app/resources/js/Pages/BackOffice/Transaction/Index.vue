...
const columns: ColumnDef<TTransaction>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => h("div", { class: "capitalize" }, row.original.customer_name),
  },
  {
    ...
    cell: ({ row }) => h("div", { class: "text-center" }, row.original.created_at),
  },
  {
    accessorKey: "item",
    header: ({ column }) => {
    ...
    cell: ({ row }) =>
      h("div", { class: "text-center" }, row.original.product_count + " Item"),
  },

  {
    ...
    cell: ({ row }) => h(TransactionStatus, { status: row.original.status }),
  },

  {
    ...
    cell: ({ row }) => h("div", { class: "text-right" }, Rupiah(row.original.total)),
  },
  {
    id: "actions",
    enableHiding: false,
    cell: ({ row }) => {
      ...
      return h(TransactionButton, {
        ...
        onUpdated: () => getTransactions(props.transactions.meta.current_page),
      });
    },
  },
];
  ...
  <div class="space-y-2 mx-auto w-full">
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        <h1 class="text-lg font-semibold md:text-xl">Data Pemesanan</h1>
        <div class="flex gap-2"></div>
      </div>
      <Alert class="bg-secondary shadow-inner shadow-gray-100 max-w-2xl">
        <AlertDescription class="text-xs">
          Halaman untuk memanajemen Pemesanan yang terdapat pada setiap Unit Layanan di
          SMKN 1 Purwosari.
        </AlertDescription>
      </Alert>
    </div>
